...
const Background = ({ children }) => {
  ...
  const delayedUpdate = useRef(debounce(count => setTrianglesCount(count), 100)).current;
  ...
  const triangles = useMemo(() => {
    return range(trianglesCount).map(key => {
      ...
      const rotate = Math.sign(Math.sin(key)) * (((Math.sin(key) + 1) * 360) % 360);

      return <Triangle left={x} top={y} rotate={rotate} />;
    });
  ...
  return (
    <>
      ...
      <BackgroundWrapper style={{ transform: `translate3d(0, -${scrollY / 2}px, 0)` }}>
        {triangles}
      </BackgroundWrapper>
    </>
  );
